...
class FSTCascade(list):
    ...
    def __init__(self, label, *fsts):
        ...
        # Segmentation units, for example, including 'll' and 'ch' in Spanish.
        self.seg_units = []
...
class FST:
    ...
    An FST is weighted if weighting is some Semiring.
    """
    ...
    def __init__(self, label, cascade=None, weighting=None):
        ...
        #String set, possibly abbreviated in FST file
        self._stringsets = {}
        ...
        # State Information
        ## Note that only one initial state is possible
    ...
    def arc_weight_jit(self, arc):
        """Return the weight for the given arc, creating it from a string if necessary."""
...
#        print('Weight to parse', weight)
    ...
    def arc_unknown(self, arc, in_side = True):
        """Is the label on in(out)_side UNKNOWN?"""
...
#            print('Deleting {}, incoming {}'.format(label, self._outgoing[label]))
        ...
        for arc in self._incoming[label]:
            if arc in self._src:    # It may have been deleted already
#self.arcs():  
                self._outgoing[self._src[arc]].remove(arc)   # First remove from other end
...
#            if arc in self.arcs():
            if arc in self._src:    # It may have been deleted already
                self._incoming[self._dst[arc]].remove(arc)   # First remove from other end
                del (self._src[arc], self._dst[arc], self._in_string[arc],
                     self._out_string[arc], self._arc_descr[arc])
    ...
    def set_final_weight(self, state, weight):
        """
        Set the given state's final weight.
        """
        if weight:
            if not self._is_final[state]:
                raise ValueError('%s is not a final state' % state)
...
#                self.del_state(state, 1)
    ...
    def copy(self, label=None):
        ...
        # Copy all state:
        fst._initial_state = self._initial_state
        # Values are lists, so we need deepcopies
    ...
    def __str__(self):
        # State labels cast with 'str' to make tuples also possible labels
    ...
    def init_weight(self):
        """
        Default initial weight for transduction.
        """
        if self.is_weighted():
    ...
    def stringify_weights(fst):
        '''Convert each weight to a string.'''
    ...
    def restore(fst_name, directory='', cascade=None, weighting=UNIFICATION_SR, seg_units=[], empty=True,
                segment=False, generate=False, create_weights=True, verbose=False):
...
#                print('Read in stringsets for', fst)
    ...
    def parse(label, s, weighting=None, cascade=None, fst=None,
        ...
        while lines:
            ...
            # Final state
            # Still need a way to read in the final weight
            m = FINAL_RE.match(line)
            if m:
                label, finalizing_string = m.groups()
                if not fst.has_state(label): fst.add_state(label)
            ...
            # Transition arc(s)
            # Changes here to allow for weight at end,
            ...
            # FST cascade to be loaded from file, composed, and concatenated in
...
#                fstpath = os.path.join(directory, fstfile)
            ...
            # FST already loaded or to load from file, to be concatenated in
            # >file<
            m = FST_RE.match(line)
            if m:
                src, dst, label, weight = m.groups()
    ...
    def _parse_arc(self, string):
        """Parse input and output strings for one arc.
...
#            print('make_mult_arcs with weight {}'.format(weight.__repr__()))
    ...
    def tree_to_fst(tree, label, cascade=None, weighting=None, lex_features=False,
                    ...
                    verbose=False):
        """Turn a letter tree into an FST.
    ...
    def _subtree_to_states(self, state, subtree, label, weighting=None, lex_features=False, dest=False,
        ...
        """subtree a dict with characters as keys, values either subtrees, (chars, feats), or [feats]. """
    ...
    def _extend_subtree(self, state, chars, features, weighting=None, lex_features=False, dest=False,
                        weight_constraint=None, verbose=False):
        """Extend a subtree (remaining chars and features)."""
        curr_state = state
        if verbose:
    ...
    def transduce(self, input, init_weight=None, split_string='',
        ...
        """Return the output for all paths through the FST for the input and initial weight."""
        if timeit:
            time1 = time.time()
        words = []
        result = []
...
#        print('transduce result: {}'.format(result))
    ...
    def consolidate(self, out_weights):
        """For transduction output list, consolidate weights for the same output."""
    ...
    def transduce_all(self, inputs_weights):
        """Transduce all input weight pairs."""
    ...
    def _transduce_match_final(self, state, input, in_pos, accum_weight, trace=0, path=[]):
        """Match final states in transduction, returning accumulated weight."""
    ...
    def step_transduce(self, input, step=True, all_paths=True, init_weight=None,
        ...
        # Outer loop; necessary for returning outputs for multiple paths
        while go_on:
            ...
            if go_on:
                ...
                if self.is_weighted():
                    # For a weighted FST, multiply in the final state's weights
                    fw = self._final_weight.get(state, None)
                    if fw:
                        accum_weight = self.weighting().multiply(accum_weight, fw)
                # Return the output side and weight for one path
                found = True
                yield ['succeed', output, accum_weight]

                # If all_paths is True, backtrack and return output for other paths
                if all_paths and frontier:
                    ...
                    output = output[:out_pos]
                    output.append(out_string)
                # Stop looking if all_paths is False or there's nothing on the frontier
                else:
                    go_on = False
